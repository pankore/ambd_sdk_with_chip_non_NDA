...
{
    return sAmebaFixedLabelCount;
}

const char * matter_get_fixed_label_name(uint8_t index)
{
    if (index >= MAX_FIXED_LABELS_COUNT || !sAmebaFixedLabel[index].isSet) {
        return NULL;
    }

    return sAmebaFixedLabel[index].key;
}
...
const char * matter_get_fixed_label_value(uint8_t index)
{
    ...
    return sAmebaFixedLabel[index].value;
}
...
{
    return sAmebaSupportedLocalesCount;
}
...
{
    return sAmebaCalendarTypeCount;
}

bool matter_get_calendar_type_value(uint8_t index, uint8_t *output)
{
    if (index >= sAmebaCalendarTypeCount || output == NULL)
    {
        return false;
    }

    if (sAmebaCalendarTypes[index] >= 0xC)
    {
        return false;
    }
...
{
    uint8_t sTestCalendarTypes[] = {0x1, 0x5, 0x9, 0xB};
